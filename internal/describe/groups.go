...
const (
	...
	GroupUseDefaultDevices = "If set to 'true' the group can notify members on their failsafe (default) devices if none of the member's other devices are available."
	...
	GroupCriteriaOperand   = "The operand to use to limit or expand the search query parameter: AND or OR. AND only returns dynamic groups that have all search terms in the name or description. OR returns dynamic groups that have any of the search terms in the name or description."
	GroupCriteriaCriterion = "Set of conditions users must satisfy for inclusion in the dynamic group."
	GroupCriterionType     = "The criterion type. Available options are: BASIC_FIELD, CUSTOM_FIELD"
	GroupCriterionField    = "The field being evaluated. Available options for a basic field are: FIRST_NAME, LAST_NAME, SITE, USER_ID, WEB_LOGIN, ROLE_NAME. The option for a custom field is the user-defined custom field."
	GroupCriterionOperand  = "How the property is being evaluated. Available options for a basic field are: EQUALS, BEGINS_WITH, ENDS_WITH, CONTAINS. Available options for a custom field are: EQUALS, BEGINS_WITH, ENDS_WITH, CONTAINS, true, false."
	GroupCriterionValue    = "The value to evaluate the criteria against."
	...
	GroupsFilterMemberLicenseType = "Returns a list of groups that contain at least one member (or a device that belongs to a user) who has the specified license type. The member does not have to be part of any shifts for the group to be included in the response. Available values are: FULL_USER, STAKEHOLDER_USER."
	...
	GroupResourceCriteriaOperand   = "Sets whether a user must match all or any of the specified criterion to be included in the group. Available values: - AND: users must match all criterion - OR: users must match at least one criterion."

	// --------------------------------------------------
	// Group Roster Resource
	// --------------------------------------------------
	// Group Roster Resource Schema
	GroupMembersResourceID         = "Unique identifier (UUID) of the group in xMatters to manage membership."
	GroupMembersResourceGroup      = "A reference to the group in which membership is being managed."
	GroupMembersResourceMembers    = "A list of members to add to the group. The members can be people, devices, or other groups. If the member is a group, the group must be a dynamic team."
	GroupMembersResourceMemberID   = "The unique identifier of the member to add to the group. The member can be a person, device, or group."
	GroupMembersResourceMemberType = "The type of the member to add to the group. Available options are: PERSON, DEVICE, GROUP."
)
