...
// DevicesDataSource defines the data-source implementation for a list of xMatters Devices.
...
func (d *DevicesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	// Get the desired list of devices from the xMatters API
	devicesReturn, err := d.client.GetDeviceList(state.APIParams(&resp.Diagnostics))
	if resp.Diagnostics.HasError() {
		return
	}
	if err != nil {
		resp.Diagnostics.Append(helpers.DatasourceErrorDiagnostic("devices", err))
		return
	}

	// Transform the API response into a Terraform state
	newState := DevicesModel{
		Devices: utils.FlattenDeviceList(&resp.Diagnostics, devicesReturn),
	}
	if resp.Diagnostics.HasError() {
		return
	}
